...
describe("Game Initialization and Functionality Tests", () => {
  ...
  test("gameInitialisation should initialize the game elements", () => {
    ...
    if (backgroundContainer) {
      expect(backgroundContainer.style.backgroundImage).toContain(
        "./mocked-background-image.webp"
      );
    } else {
  ...
  test("menuPopup should display the popup and set up close button events", () => {
    const { openPopup } = menuPopup();
    openPopup();

    expect(document.querySelector(".popup").classList.contains("hide")).toBe(
      false
    );
  ...
  test("gameOver should display the game over popup", () => {
    gameOver();
    expect(
      document.querySelector(".popup-game-over").classList.contains("hide")
    ).toBe(false);
  });
});
